...
/* The photograph of each colourway, so a quote shows the colour that was
   picked rather than whichever one the supplier shot.
​
   The catalogue's own `colours[].image` carries the per-colourway ART, which
   jt-catalog.php derives from a product's variation stages — so only the 29
   products that went through tools/product-art/ have one, 409 colourways of
   2212. Everything else fell through to the product thumbnail, which is a
   photo of ONE colourway: quote CDAF2C showed a white-grey Gildan 18600 under
   the word "Black", because #23's thumbnail is its Ash shot and Ash is what
   all eighteen of its colours displayed.
​
   Generated by tools/colour-photos.js from the S&S `colorFrontImage` field.
   It is a FILE rather than a column because the catalogue arrives over HTTP
   from jt-catalog.php, which lives in the Lumise image and not on its volume,
   so a change there is wiped by the next deploy.
​
   Missing or unreadable is not fatal — every colourway then falls back to the
   thumbnail, exactly as it did before this existed. */
let COLOUR_PHOTOS = {};
try {
  COLOUR_PHOTOS = require('./data/colour-photos.json');
} catch (err) {
  console.error('colour photos unavailable, falling back to product thumbnails:', err.message);
}
const colourPhoto = (productId, colourName) => {
  const forProduct = COLOUR_PHOTOS[String(productId)];
  const url = forProduct && forProduct[String(colourName)];
  return typeof url === 'string' && /^https:\/\/cdn\.ssactivewear\.com\//.test(url) ? url : '';
};
...
​
      ...
      /* Then the supplier's photograph of that colourway. Second, not first:
         the art above is cut out and aligned to the garment, and where it
         exists it is the better picture. This is the one that covers the other
         74 products — the reason the fallback below was being reached at all. */
      if (!images.length && colourRow && prod) {
        const photo = colourPhoto(prod.id, colourRow.name);
